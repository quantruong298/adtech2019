    ...
    public function ads($id, $param, Request $request)
    {
        ...
        for ($i = 0; $i < count($email); $i++)
        {
            $ads[$i] = DB::table('ads')
                ...
                ->select('ads.id','ads.name', 'ads_detail.status', 'campaigns.name as campaign_name', 'ad_groups.name as ad_group_name', 'flags.flag_name', 'ads_detail.creative_preview', 'creative_types.name as create_type_name', 'ads_detail.spent', 'ads_detail.click_through_rate', 'ads_detail.cost_bidding', 'ads_detail.period_from', 'ads_detail.period_to', 'ads_detail.ads_period_budget', 'ads_detail.ads_period_budget_from', 'ads_detail.ads_period_budget_to', 'ads_detail.std_daily_budget', 'std_bidding_methods.name as std_bidding_methods_name')
                ->where('advertiser_email', $email[$i])
